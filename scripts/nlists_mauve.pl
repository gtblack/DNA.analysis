...
open PROT, "nucleotide.mauve.aligned.txt";
while (<PROT>) {
  chomp;
  my $line = $_;
  if ($flag < 0) {
    ...
    foreach my $key (1..$num_indels{$ref_id}) {
      my $ntag = "${ref_id}_$key";
      $indices{$ntag} = $sorted_indices[$key - 1];
      $indel_types{$ntag} = $index_sorter{$sorted_indices[$key - 1]};
      if ($indel_types{$ntag} eq 'm') {
	$ref_seq{$ntag} =
	  substr ($line, ($indices{$ntag} + $adj_loc{$shot_id}), 1);
	$shot_seq{$ntag} = 
	  substr ($shot_full, ($indices{$ntag} + $adj_loc{$shot_id}), 1);
      } else {
	my $ref_indic = 
	  substr ($line, ($indices{$ntag} + $adj_loc{$shot_id}), 1);
	my $blen = $blocks{$sorted_indices[$key - 1]};
	if ($ref_indic eq '-') {
	  $ref_seq{$ntag} = $ref_indic;
	  $shot_seq{$ntag} = 
	    substr ($shot_full, ($indices{$ntag} + $adj_loc{$shot_id}), $blen);
	} else {
	  $ref_seq{$ntag} =
	    substr ($line, ($indices{$ntag} + $adj_loc{$shot_id}), $blen);
	  $shot_seq{$ntag} = '-';
	}
      }
      ...
      # to proper locations.
      my $front_part = substr $line, 0, ($indices{$ntag} + $adj_loc{$shot_id});
      my $blanks = scalar (&find_index($front_part, '-'));
      my $adjust = $adj_loc{$shot_id} - $blanks + $indices{$ntag};
      if ($ref_order{$ref_id} eq '+') {
	$locations{$ntag} = $ref_st{$ref_id} + $adjust;
      } else {
	$locations{$ntag} = $ref_end{$ref_id} - $adjust;
      }
    }
    %blocks = ();
    $flag = -2;
