...
while (<ALL>) {
  ...
  my @block = split /\t/, $line;
  if ($block[0] ne 'Query') {
    $all_pairs{$block[0]} = $line;
  }
}

open OUTPUT, ">BLASTp_exclusive.txt";
print OUTPUT "Query\t\t\tDatabase\tScore\tE Value\n";
foreach my $key (sort keys %all_pairs) {
  if (!(defined $anchor_pairs{$key}) && !(defined $align_pairs{$key})) {
    print OUTPUT "$all_pairs{$key}\n";
  }
}
close OUTPUT;
