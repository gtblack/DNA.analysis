...
open OUTPUT, ">PI25611.ordered.aa.fasta";
foreach my $stag (@shot_tags) {
  print OUTPUT $shot_amino{$stag};
}
close OUTPUT;

open OUTPUT, ">PI17.ordered.faa";
...
open OUTPUT, ">PI25611.ordered.na.fasta";
foreach my $stag (@shot_tags) {
  print OUTPUT $shot_nucl{$stag};
}
close OUTPUT;

open OUTPUT, ">PI17.ordered.fna";
foreach my $rtag (@ref_tags) {
  print OUTPUT $ref_nucl{$rtag};
}
close OUTPUT;

open OUTPUT, ">logs.txt";
foreach my $key (keys %ref_nucl) {
  print OUTPUT "$key\n$ref_nucl{$key}\n";
}
close OUTPUT;
